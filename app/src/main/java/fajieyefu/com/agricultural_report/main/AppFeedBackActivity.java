...
public class AppFeedBackActivity extends BaseActivity {
	...
	public void onClick() {
		...
		if (TextUtils.isEmpty(contentText)) {
			Toast.makeText(AppFeedBackActivity.this, "请输入您的反馈意见", Toast.LENGTH_SHORT).show();
			return;
		}
		if (TextUtils.isEmpty(phoneNum) || !BeanUtil.isMobileNO(phoneNum)) {
			Toast.makeText(AppFeedBackActivity.this, "请输入合法的手机号码", Toast.LENGTH_SHORT).show();
		}
		postToServer();
	}
	...
	private class MyCallBack extends RespnseCallBack {
		...
		public void onError(Call call, Exception e, int id) {
			Toast.makeText(AppFeedBackActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onResponse(ResponseBean response, int id) {
			if (response.getCode()==0){
				Toast.makeText(AppFeedBackActivity.this, "意见反馈成功，非常感谢！", Toast.LENGTH_SHORT).show();
			}else {
				Toast.makeText(AppFeedBackActivity.this, response.getMsg(), Toast.LENGTH_SHORT).show();
			}
